package edu.towson.cosc.cosc455.afabia.lab4.afabian.lab4

import java.lang

import edu.towson.cosc.cosc455.afabia.lab4.afabian
import edu.towson.cosc.cosc455.afabian.lab4
...
class SyntaxAnalyzer {
  ...
  def Verb() = {
    if (VERBS contains Compiler.currentToken)
      afabian.lab4.Compiler.Scanner.getNextToken()
    else {
      println("SYNTAX ERROR - A verb was expected when '" + afabian.lab4.Compiler.currentToken + "' was found.")
      setError()
    }
  }
  ...
  def Noun() = {
    if (NOUNS contains afabian.lab4.Compiler.currentToken)
      afabian.lab4.Compiler.Scanner.getNextToken()
    else {
        println("SYNTAX ERROR - A noun was expected when '" + afabian.lab4.Compiler.currentToken + "' was found.")
        setError()
      }
  }
  ...
  def Article() = {
    if (ARTICLES contains afabian.lab4.Compiler.currentToken)
      afabian.lab4.Compiler.Scanner.getNextToken()
    else {
        println("SYNTAX ERROR - An article was expected when '" + afabian.lab4.Compiler.currentToken + "' was found.")
        setError()
      }
  }
}
